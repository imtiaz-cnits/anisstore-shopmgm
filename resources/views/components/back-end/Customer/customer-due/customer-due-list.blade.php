        ...
        <div class="bredcam">
            <div class="bredcam-title">
                <h1 class="d-flex align-items-center gap-2">
                    <i class="fa-solid fa-file-invoice-dollar text-success"></i> Customer Due List (কাস্টমার বকেয়া তালিকা)
                </h1>
            </div>
        </div>
        <div class="data-table">
            <div class="card">
                <div class="card-body">
                        ...
                        <div class="d-flex flex-wrap align-items-center gap-2 mb-3">
                            <div class="input-group" style="max-width: 320px;">
                                <input type="text" id="searchInput" class="form-control"
                                    placeholder="Search Customer Due..." />
                            </div>
                    ...
                    <!-- Desktop Table View (Shown on Desktop Screens >= 768px) -->
                    <div class="table-responsive d-none d-md-block">
                        <table id="printTable" class="table align-middle table-hover mb-0">
                            <thead class="bg-light">
                                <tr class="text-uppercase small fw-bold text-muted">
                                    ...
                                    <th class="text-end">Order Due</th>
                                    <th class="text-end pe-3">Total Due</th>
                                </tr>
                            </thead>
                            <tbody id="tableList"></tbody>
                            <tfoot class="bg-light fw-bold">
                                <tr>
                                    <td colspan="5" class="text-end text-uppercase">Total Due Summary:</td>
                                    <td id="total_previous_due_amount" class="text-end text-dark">৳ 0.00</td>
                                    <td id="total_order_due_amount" class="text-end text-dark">৳ 0.00</td>
                                    <td id="total_total_due_amount" class="text-end text-danger pe-3 fs-6">৳ 0.00</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
...
async function getList() {
    try {
        ...
        if (res.data.status === "success" && Array.isArray(res.data.CustomerData)) {
            res.data.CustomerData.forEach((item, index) => {
                ...
                const row = `
                    <tr>
                        ...
                        <td>
                            <div id="action_btn_wrap" class="d-flex align-items-center gap-1">
                                <a href="/customer/profile/${item['id']}" onclick="window.location.href='/customer/profile/${item['id']}';" class="link text-primary me-1" title="কাস্টমার প্রোফাইল দেখুন">
                                    <i class="fa-solid fa-circle-user fs-5"></i>
                                </a>
                                <a data-id="${item['id']}" href="#" class="link edit-link" data-bs-toggle="modal" data-bs-target="#exampleModal" title="এডিট">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>
                                <a href="#" data-id="${item['id']}" class="link custom-delete-modal-btn" data-bs-toggle="modal" data-bs-target="#confirmationModal" title="ডিলেট">
                                    <i class="fa-solid fa-trash"></i>
                                </a>
                            </div>
                        </td>
                        <td>
                            <a href="/customer/profile/${item['id']}" onclick="window.location.href='/customer/profile/${item['id']}';" class="badge bg-success-subtle text-success border border-success-subtle font-monospace text-decoration-none py-1 px-2 d-inline-block" style="cursor: pointer;" title="কাস্টমার প্রোফাইল দেখুন">
                                <i class="fa-solid fa-user me-1"></i>${item.customer_id || 'N/A'}
                            </a>
                        </td>
                        <td>
                            <a href="/customer/profile/${item['id']}" onclick="window.location.href='/customer/profile/${item['id']}';" class="fw-bold text-dark text-decoration-none d-inline-block" style="cursor: pointer;" title="কাস্টমার প্রোফাইল দেখুন">
                                ${item.customer_name || ''}
                            </a>
                        </td>
                        <td class="fw-semibold text-secondary">${item.mobile || ''}</td>
                        <td class="text-end fw-semibold">৳ ${previous_due.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td>
                        <td class="text-end fw-semibold">৳ ${order_due.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td>
                        <td class="text-end pe-3 fw-bold text-danger">৳ ${total_due.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td>
                    </tr>
                `;
                ...
                const mobileCard = `
                    <div class="customer-mobile-card card border shadow-sm rounded-4 mb-3 p-3 position-relative bg-white">
                        <div class="d-flex align-items-center justify-content-between pb-2 mb-2 border-bottom">
                            ...
                            <div>
                                <span class="badge bg-success-subtle text-success border border-success-subtle px-2 py-1 fw-bold" style="font-size: 10px; border-radius: 12px;">
                                    Active
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="bg-light p-2 rounded-3 my-2 border" style="border-color: #f1f5f9 !important;">
                            ...
                            <div class="d-flex align-items-center justify-content-between mb-1">
                                <span class="text-muted small fw-bold" style="font-size: 11px;">অর্ডার বকেয়া (Order Due):</span>
                                <span class="fw-bold small text-dark">৳ ${order_due.toFixed(2)}</span>
                            </div>
                            <div class="d-flex align-items-center justify-content-between pt-1 border-top">
                                ...
                                <span class="fw-bold fs-6 ${total_due > 0 ? 'text-danger' : 'text-dark'}">৳ ${total_due.toFixed(2)}</span>
                            </div>
                        </div>

                        <div class="d-flex align-items-center justify-content-between pt-2 mt-1 border-top">
                            <a href="/customer/profile/${item['id']}" onclick="window.location.href='/customer/profile/${item['id']}';" class="btn btn-sm btn-outline-primary rounded-pill px-3 fw-bold" style="font-size: 11px;">
            ...
            $("#total_previous_due_amount").text('৳ ' + totalPreviousDue.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2}));
            $("#total_order_due_amount").text('৳ ' + totalOrderDue.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2}));
            $("#total_total_due_amount").text('৳ ' + totalTotalDue.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2}));

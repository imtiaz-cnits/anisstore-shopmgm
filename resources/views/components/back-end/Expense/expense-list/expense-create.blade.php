<style>
    /* Ultra-Responsive Modern Mobile & Desktop Styling for Create Expense Modal */
    .financemodal {
        position: fixed;
        top: 0;
        left: 0;
    ...
    @media (max-width: 768px) {
        ...
        .expense-create-modal-body {
            ...
            max-height: calc(96vh - 65px) !important;
        }

        .expense-top-action-btns {
            flex-direction: column !important;
            gap: 8px !important;
        }

        .expense-top-action-btns .btn {
            width: 100% !important;
...
<div class="main-content" id="myModal">
    <div class="page-content" id="signup">
        ...
        <section id="createProduct" class="financemodal">
            <div class="modal-content expense-create-modal-content shadow-lg border-0">
                <div class="modal-header text-white py-3 px-3 px-md-4" style="background: linear-gradient(135deg, #15803d 0%, #16a34a 100%);">
                    <h5 class="modal-title fw-bold mb-0 text-white fs-6 fs-md-5">
                    ...
                    <form id="expenseForm" onsubmit="return Save(event)">
                        ...
                        <div class="row g-3 mb-3 bg-white p-3 rounded-3 shadow-sm border">
                            <div class="col-12 col-md-5">
                                ...
                                <input type="date" class="form-control fw-bold text-dark" id="ExpenseDate" required style="height: 44px; border-radius: 8px;" />
                            </div>
                            <div class="col-12 col-md-7 d-flex align-items-end gap-2 expense-top-action-btns">
                                <button type="button" class="btn btn-outline-success fw-bold w-100 d-flex align-items-center justify-content-center gap-1 newbrand-open" onclick="openBrandModal()" style="height: 44px; border-radius: 8px;">
                                    <i class="fa-solid fa-folder-plus"></i> + নতুন টাইপ তৈরি করুন
                                </button>
                                <button type="button" class="btn btn-outline-primary fw-bold w-100 d-flex align-items-center justify-content-center gap-1" onclick="openStaffQuickModal()" style="height: 44px; border-radius: 8px;">
                        ...
                        <div class="bg-white p-3 rounded-3 shadow-sm border mb-4">
                            <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center justify-content-between mb-3 border-bottom pb-2 gap-2 expense-header-info-badge">
                                <h6 class="fw-bold text-dark mb-0" style="font-size: 13.5px;">
                                    <i class="fa-solid fa-list-check text-success me-1"></i> এক্সপেন্স টাইপ সিলেক্ট করুন এবং পরিমাণ বসান:
                                </h6>
                                <small class="text-muted" style="font-size: 11px;"><i class="fa-solid fa-circle-info text-primary me-1"></i> সেলারি দেওয়ার সময় স্টাফ সিলেক্ট করুন</small>
                            </div>
                        ...
                        <!-- Action Buttons -->
                        <div class="d-flex align-items-center justify-content-end gap-2 expense-footer-btns">
        ...
        <div class="newbrand" id="addBrandModal" style="z-index: 999999;">
            <div class="newbrand-content shadow-lg border-0">
                <h4 class="fw-bold text-success mb-3" style="font-size: 18px;"><i class="fa-solid fa-folder-plus me-2"></i> নতুন এক্সপেন্স টাইপ</h4>
        ...
        <!-- Edit Expense Type Modal Start -->
        <div class="newbrand" id="editBrandModal" style="z-index: 9999999;">
            <div class="newbrand-content shadow-lg border-0">
                <h4 class="fw-bold text-primary mb-3" style="font-size: 18px;"><i class="fa-solid fa-pen-to-square me-2"></i> এডিট এক্সপেন্স টাইপ</h4>
                <form onsubmit="updateExpenseTypeSubmit(event)">
                    <input type="hidden" id="EditExpenseTypeId" />
                    <div class="form-group mb-3">
                        <label class="form-label small fw-bold text-secondary">টাইপের নাম (Expense Type Name) *</label>
                        <input type="text" id="EditExpenseTypeName" class="form-control fw-bold" placeholder="যেমন: দোকান ভাড়া, বিদ্যুৎ বিল" required />
                    </div>
                    <div class="form-group mb-4">
                        <label class="form-label small fw-bold text-secondary">স্ট্যাটাস (Status) *</label>
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        <button type="button" class="btn btn-secondary px-3" onclick="closeEditBrandModal()">ক্যান্সেল</button>
        ...
        <div class="newbrand" id="addStaffQuickModal" style="z-index: 999999;">
            <div class="newbrand-content shadow-lg border-0">
                <h4 class="fw-bold text-primary mb-3" style="font-size: 18px;"><i class="fa-solid fa-user-plus me-2"></i> নতুন স্টাফ যুক্ত করুন</h4>
...
<script>
    ...
    function renderExpenseTypeCheckboxes() {
        ...
        globalExpenseTypes.forEach(type => {
            ...
            html += `
                <div class="expense-type-row border rounded-3 p-3 transition-all mb-2" id="type-row-${type.id}">
                    <div class="d-flex align-items-center justify-content-between gap-2">
                        <div class="form-check mb-0 d-flex align-items-center gap-2">
                            ...
                            <label class="form-check-label fw-bold text-dark mb-0 d-flex align-items-center flex-wrap gap-1" for="chk-${type.id}" style="cursor: pointer; font-size: 14px;">
                                ...
                                ${isSalary ? '<span class="badge bg-primary-subtle text-primary border border-primary-subtle px-2 py-1 small fw-bold" style="font-size: 10px;">👨‍💼 Staff Salary</span>' : ''}
                            </label>
                        </div>
                        <div class="d-flex align-items-center gap-1 flex-shrink-0">
                            <button type="button" class="btn btn-sm btn-light text-primary border rounded-circle p-0 d-flex align-items-center justify-content-center shadow-xs" onclick="openEditBrandModal(${type.id}, '${safeName}')" title="এডিট টাইপ" style="width: 34px; height: 34px;">
                                <i class="fa-solid fa-pen-to-square fs-6"></i>
                            </button>
                            <button type="button" class="btn btn-sm btn-light text-danger border rounded-circle p-0 d-flex align-items-center justify-content-center shadow-xs" onclick="deleteExpenseTypeConfirm(${type.id}, '${safeName}')" title="ডিলেট টাইপ" style="width: 34px; height: 34px;">
                    ...
                    <div class="type-input-group mt-3 d-none pt-2 border-top" id="input-group-${type.id}">
                        <div class="row g-2">
                            <div class="col-12 col-md-4">
                                ...
                                <div class="input-group input-group-sm">
                                    <select class="form-select staff-select fw-bold text-dark" id="staff-${type.id}" style="height: 40px; font-size: 13px;">
                                        <option value="">-- স্টাফ নির্বাচন করুন --</option>
                                        ${globalStaffList.map(s => `<option value="${s.id}">${s.name} (${s.mobile || 'Staff'})</option>`).join('')}
                                    </select>
                                    <button class="btn btn-outline-primary px-2" type="button" onclick="openStaffQuickModal()" title="নতুন স্টাফ যুক্ত করুন" style="height: 40px;">
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <input type="number" step="any" class="form-control form-control-sm amount-input fw-bold text-success" id="amount-${type.id}" placeholder="0.00" style="height: 40px; font-size: 15px;" />
                            </div>
                            <div class="col-12 col-md-4">
                                <label class="form-label small fw-bold text-secondary mb-1">বিবরণ / নোট (Details)</label>
                                <input type="text" class="form-control form-control-sm details-input" id="details-${type.id}" placeholder="${isSalary ? 'মাসের বেতন / অ্যাডভান্স' : 'খরচের বিবরণ লিখুন...'}" style="height: 40px; font-size: 13px;" />
                            </div>
                        </div>
                    </div>
                </div>
    ...
    async function updateExpenseTypeSubmit(event) {
        ...
        try {
            ...
            if (res.data.status === 'success') {
                if (typeof successToast === "function") successToast("টিপস: এক্সপেন্স টাইপ আপডেট হয়েছে!");
                closeEditBrandModal();
                await loadExpenseTypesAndStaff();
                if (typeof fetchExpenseList === "function") fetchExpenseList();
            } else {
    ...
    async function deleteExpenseTypeConfirm(id, name) {
        if (!confirm(`আপনি কি নিশ্চিত যে "${name}" টাইপটি মুছে ফেলতে চান?`)) {
            return;
        }
        ...
        try {
            ...
            if (res.data.status === 'success') {
                if (typeof successToast === "function") successToast("টিপস: এক্সপেন্স টাইপ মুছে ফেলা হয়েছে!");
                await loadExpenseTypesAndStaff();
                if (typeof fetchExpenseList === "function") fetchExpenseList();
            } else {

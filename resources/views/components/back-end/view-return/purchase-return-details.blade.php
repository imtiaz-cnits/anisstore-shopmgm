...
@section('title', 'ক্রয় ফেরত - আনিস ষ্টোর')
@section('content')

    <!-- Scoped Styles for Modern Purchase Return Invoice -->
    <style>
         .return-page-header {
            max-width: 98%;
        ...
        <div class="page-content">
            ...
            <div class="return-page-header d-flex align-items-center justify-content-between flex-nowrap gap-2">
                ...
                <div class="d-flex align-items-center gap-2 flex-nowrap">
                    ...
                    <button type="button" class="return-submit-btn" onclick="ReturnPurchaseSave(event)">
                        <i class="fa-solid fa-rotate-left"></i>
            ...
            <div class="invoice-container">
                <div class="billing-section">
                    <div class="wrapper">
                        <div class="billing-to">
                            <h3 class="fw-bold mb-1" style="font-size: 14px; color: #8C56D4;">সাপ্লায়ার তথ্য (Billed to)</h3>
                            <p class="mb-1"><strong id="SupplierName" style="font-size: 14px;">{{ $purchase->supplier->name ?? 'N/A' }}</strong></p>
                            <p id="SupplierAddress" class="text-muted mb-1">{{ $purchase->supplier->address ?? 'ঠিকানা নেই' }}</p>
                            <p class="text-muted mb-0">মোবাইল: <span id="SupplierMobile" class="fw-semibold text-dark">{{ $purchase->supplier->mobile ?? 'N/A' }}</span></p>
                        </div>
                        ...
                        <div class="invoice-wrapper">
                            <table>
                                <tr>
                                    ...
                                    <td id="referance_no" class="fw-bold text-dark">{{ $purchase->purchase_id ?? ('#PurID' . str_pad($purchase->id, 5, '0', STR_PAD_LEFT)) }}</td>
                                </tr>
                                <tr>
                                    <td class="date">ইনভয়েস তারিখ:</td>
                                    <td id="purchase_date">{{ \Carbon\Carbon::parse($purchase->date ?? $purchase->created_at)->format('d-m-Y') }}</td>
                                </tr>
                                @if(!empty($purchase->referance_no))
                                <tr>
                                    <td class="number">রেফারেন্স নং:</td>
                                    <td class="fw-semibold text-secondary">{{ $purchase->referance_no }}</td>
                                </tr>
                                @endif
                            </table>
                        </div>
                    ...
                    <div class="logo-wrapper">
                        ...
                        <div class="button mt-2">
                            <button class="return-submit-btn" onclick="ReturnPurchaseSave(event)">
                                <i class="fa-solid fa-rotate-left"></i>
                                <span>ফেরত নিশ্চিত করুন</span>
                            </button>
                        </div>
                    ...
                    <strong style="display: none" id="PurchaseID">{{ $purchase->id ?? 'N/A' }}</strong>
                    <strong style="display: none" id="SupplierID">{{ $purchase->supplier->id ?? 'N/A' }}</strong>
                        ...
                        <tbody id="purchase_order_details">
                            @if ($purchase->orderDetails && $purchase->orderDetails->isNotEmpty())
                                @foreach ($purchase->orderDetails as $key => $detail)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td class="text-start fw-semibold">{{ $detail->product->product_name ?? 'N/A' }}</td>
                                        <td>{{ $detail->quantity ?? 'N/A' }}</td>
                                        <td class="fw-bold">৳{{ number_format((float)($detail->cost_price ?? 0), 2) }}</td>
                                        <td>
                            ...
                            <tr class="summary-row d-none d-md-table-row">
                                <td colspan="3" rowspan="4" id="payment_status" class="full-paid align-middle text-center">
                                    @if ($dueAmount <= 0)
                                ...
                                <td class="amount fw-bold text-dark"><span id="sub_total">৳{{ number_format((float)$subTotal, 2) }}</span></td>
                            </tr>
                            <tr class="summary-row d-none d-md-table-row">
                                <td colspan="2" class="amount_text text-success fw-semibold"><span>পরিশোধ (Paid):</span></td>
                                <td class="amount text-success fw-bold"><span id="paidamount">৳{{ number_format((float)$paidAmount, 2) }}</span></td>
                            </tr>
                            <tr class="summary-row d-none d-md-table-row">
                                <td colspan="2" class="amount_text text-danger fw-semibold"><span>বর্তমান বকেয়া (Due):</span></td>
                                <td class="amount text-danger fw-bold"><span>৳{{ number_format((float)$dueAmount, 2) }}</span></td>
                            </tr>
                            <tr class="summary-row d-none d-md-table-row">
                                <td colspan="2" class="amount_text fw-bold">মোট প্রদেয় (Total Payable):</td>
                                <td id="total_payable_amount" class="amount fw-bold text-danger">৳{{ number_format((float)$totalPayableAmount, 2) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>

                <!-- Dedicated Mobile Calculation Summary Card -->
                <div class="mobile-invoice-summary-card d-block d-md-none mt-3 p-3 rounded-3 border">
                    ...
                    <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                        ...
                        <span class="fw-bold text-dark" style="font-size: 14px;">৳{{ number_format((float)$subTotal, 2) }}</span>
                    </div>

                    <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                        <span class="text-success fw-bold" style="font-size: 13px;">পরিশোধ (Paid):</span>
                        <span class="fw-bold text-success" style="font-size: 14px;">৳{{ number_format((float)$paidAmount, 2) }}</span>
                    </div>

                    <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                        <span class="text-danger fw-bold" style="font-size: 13px;">বর্তমান বকেয়া (Due):</span>
                        <span class="fw-bold text-danger" style="font-size: 14px;">৳{{ number_format((float)$dueAmount, 2) }}</span>
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3 rounded-2 bg-total-box" style="background: #fef2f2; border: 1.5px dashed #fca5a5; padding: 14px 16px !important;">
                        <span class="fw-bold text-danger" style="font-size: 13.5px;">সর্বমোট প্রদেয় (Total Payable):</span>
                        <span class="fw-bold text-danger" style="font-size: 15px;">৳{{ number_format((float)$totalPayableAmount, 2) }}</span>
                    </div>
                </div>
    ...
    <script>
        function handleReturnPageBack() {
            if (document.referrer && document.referrer.includes(window.location.host) && document.referrer !== window.location.href) {
                ...
                window.location.href = '/admin-dashboard-Purchase';
            }
        }
            ...
            try {
                ...
                if (selectedProducts.length === 0) {
                    if (typeof errorToast === 'function') {
                        errorToast('অনুগ্রহ করে ফেরত দেওয়ার জন্য অন্তত একটি পণ্য সিলেক্ট করুন।');
                    } else {
                ...
                if (res.data.status === "success") {
                    if (typeof successToast === 'function') {
                        successToast(res.data.message || 'পণ্য সফলভাবে ফেরত নেওয়া হয়েছে।');
                    } else {
                    ...
                    if (typeof errorToast === 'function') {
                        errorToast(res.data.message || 'পণ্য ফেরত প্রক্রিয়ায় ত্রুটি হয়েছে।');
                    } else {
                ...
                if (e.response && e.response.status) {
                    ...
                    if (typeof errorToast === 'function') {
                        errorToast('পণ্য ফেরত করার সময় ত্রুটি ঘটেছে।');
                    } else {

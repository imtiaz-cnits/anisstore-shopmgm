...
<script>
                ...
                <tr>
                    ...
                    <td class="px-3 text-center">
                        <div class="d-inline-flex align-items-center gap-1">
                            <button onclick="openProductUpdateModal('${item.id}')" class="btn btn-sm btn-outline-primary shadow-sm px-2 py-1" title="এডিট করুন" style="border-radius: 6px;">
                ...
                <div class="card border-0 shadow-sm rounded-3 mb-3 p-3 position-relative" style="border-left: 4px solid ${isZeroOrNeg ? '#dc2626' : '#d97706'} !important; background: #ffffff;">
                    ...
                    <div class="d-flex align-items-center justify-content-between mt-2 pt-2 border-top">
                        ...
                        <div class="d-flex align-items-center gap-1">
                            <button onclick="openProductUpdateModal('${item.id}')" class="btn btn-sm btn-outline-primary px-3 py-1 fw-bold" style="border-radius: 6px; font-size: 11px;">
    ...
    function filterLowStockTable() {
        const rawSearch = document.getElementById('lowStockSearch').value || '';
        const statusVal = document.getElementById('stockStatusFilter').value;

        const searchEng = typeof banglaToEngNum === 'function' ? banglaToEngNum(rawSearch) : rawSearch;
        const searchBn  = typeof engToBanglaNum === 'function' ? engToBanglaNum(rawSearch) : rawSearch;

        const searchWordsRaw = String(rawSearch).toLowerCase().replace(/[\u200B-\u200D\uFEFF]/g, "").trim().split(" ").filter(Boolean);
        const searchWordsEng = String(searchEng).toLowerCase().replace(/[\u200B-\u200D\uFEFF]/g, "").trim().split(" ").filter(Boolean);
        const searchWordsBn  = String(searchBn).toLowerCase().replace(/[\u200B-\u200D\uFEFF]/g, "").trim().split(" ").filter(Boolean);
        ...
        const filtered = rawLowStockData.filter(item => {
            ...
            const unit = String(item.unit_name || "").toLowerCase();
            const codeRaw = String(item.product_code || "");
            const codeEng = typeof banglaToEngNum === 'function' ? banglaToEngNum(codeRaw) : codeRaw;
            const codeBn  = typeof engToBanglaNum === 'function' ? engToBanglaNum(codeRaw) : codeRaw;

            const corpus = `${name} ${category} ${brand} ${unit} ${codeRaw.toLowerCase()} ${codeEng.toLowerCase()} ${codeBn.toLowerCase()}`;

            return searchWordsRaw.every((word, idx) => {
                const wEng = searchWordsEng[idx] || word;
                const wBn  = searchWordsBn[idx]  || word;
                return corpus.includes(word) || corpus.includes(wEng) || corpus.includes(wBn);
            });
        });

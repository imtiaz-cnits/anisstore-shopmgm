...
class OrderController extends Controller
{
    ...
    public function OrderCreate(Request $request)
    {
        ...
        try {
            ...
            $subTotal = (float) ($request->sub_total ?? 0);
            $discountAmount = (float) ($request->discount_amount ?? 0);
            $returnAdj = (float) ($request->return_adjustment_amount ?? 0);
            $netPayable = max(0, $subTotal - $discountAmount - $returnAdj);
            
            $paymentMethod = $request->payment_method ?: 'cash';
            $paidAmount = (float) ($request->paid_amount ?? 0);

            // Fallback for cash payment when paid_amount was not provided
            if ($paidAmount == 0 && $paymentMethod !== 'due') {
            ...
            if (empty($paymentStatus) || $paymentStatus === 'বাকী') {
            ...
            $orderData = [
                ...
                'previous_due_amount' => (float) ($request->previous_due_amount ?? 0),
            ...
            if (Schema::hasColumn('orders', 'return_adjustment_amount')) {
                $orderData['return_adjustment_amount'] = $returnAdj;
            }

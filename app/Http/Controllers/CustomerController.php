    ...
    try {
        ...
        $invoices = Order::where('customer_id', $id)
                        ...
                        ->map(function($order) {
                            ...
                            $netBill = max(0, $subTotal - $discountAmount - $returnAdj);
                            $effectivePaid = $paidAmount + $returnAdj;
                            $dueAmount = max(0, $netBill - $paidAmount);
                            $paymentStatus = ($effectivePaid >= $netBill && $netBill > 0) ? 'Fully Paid' : ($effectivePaid > 0 ? 'Partial Paid' : 'Unpaid');

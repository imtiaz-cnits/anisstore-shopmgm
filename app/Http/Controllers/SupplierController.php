...
class SupplierController extends Controller
{
    ...
    public function SupplierList()
    {
        try {
            ...
            // Calculate purchase dues per supplier
            ...
            $SupplierData = Supplier::all()->map(function($s) use ($hasReturnAdjCol, $hasPurchaseReturns, $dueAmounts) {
                ...
                $prevDue = (float) ($s->purchase_payable_amount ?? 0);
                $invDue  = (float) ($dueAmounts[$s->id] ?? 0);
                $combinedDue = $prevDue + $invDue;

                $s->previous_due = $prevDue;
                $s->total_due_amount = $invDue;
                $s->combined_due = $combinedDue;
                $s->purchase_payable_amount = $combinedDue; // Combined (12000 + 320 = 12320)
...
public function SupplierDueList()
{
    try {
        ...
        // Self-repair purchases table zero grand_subtotal records
        try {
            $zeroPurchases = Purchase::with('orderDetails')->where('grand_subtotal', '<=', 0)->get();
            foreach ($zeroPurchases as $zp) {
                if ($zp->orderDetails && $zp->orderDetails->count() > 0) {
                    $calcSubtotal = (float) $zp->orderDetails->sum(function($d) {
                        return ((float)$d->cost_price) * ((float)$d->quantity);
                    });
                    if ($calcSubtotal > 0) {
                        $calcDue = max(0, $calcSubtotal - (float)($zp->paid_amount ?? 0));
                        $zp->update([
                            'grand_subtotal' => $calcSubtotal,
                            'purchase_payable_amount' => $calcSubtotal,
                            'due_amount' => $calcDue
                        ]);
                    }
                }
        ...
        $SupplierData = $suppliers->map(function ($supplier) use ($dueAmounts) {
            $prevDue = (float) ($supplier->purchase_payable_amount ?? 0);
            $invDue  = (float) ($dueAmounts[$supplier->id] ?? 0);
            $totalCombinedDue = $prevDue + $invDue;
            ...
            return [
                ...
                'purchase_payable_amount' => $totalCombinedDue, // Combined Previous Due + Invoice Due (12000 + 320 = 12320)
                'previous_due' => $prevDue,
                'total_due_amount' => $invDue,
                'combined_due' => $totalCombinedDue,
                'status' => $supplier->status,
            ];
        })
        ->filter(function ($item) {
            return (float) $item['purchase_payable_amount'] > 0;
        })
        ->values(); 
        ...
        try {
            ...
            $purchases = Purchase::with(['orderDetails.product', 'paymentDetails'])
                ...
                ->map(function ($purchase) {
                    ...
                    // Self-healing fallback if grand_subtotal was saved as 0 in DB
                    if ($grandTotal <= 0 && $purchase->orderDetails && $purchase->orderDetails->count() > 0) {
                        $grandTotal = (float) $purchase->orderDetails->sum(function($detail) {
                            return ((float) $detail->cost_price) * ((float) $detail->quantity);
                        });
                    }
